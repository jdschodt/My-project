 ...
 * CorrectionPilot pilot = new CorrectionPilot(chassis);
 ...
 * 
 **/
public class CorrectionPilot implements ArcRotateMoveController {
   ...
   */
  public CorrectionPilot(final double wheelDiameter, final double trackWidth, final RegulatedMotor leftMotor,
	      final RegulatedMotor rightMotor, GyroSensor gyro ) {
   ...
   *          backward.
   */
  public CorrectionPilot(final double leftWheelDiameter, final double rightWheelDiameter, final double trackWidth,
    ...
    this(new WheeledChassis(new Wheel[] { 
        ...
        WheeledChassis.modelWheel(rightMotor, rightWheelDiameter).offset(-trackWidth / 2).invert(reverse) }, WheeledChassis.TYPE_DIFFERENTIAL),gyro);
  }
   ...
   */
  public CorrectionPilot(Chassis chassis,GyroSensor agyro) {
    this.chassis = chassis;
    linearSpeed = chassis.getLinearSpeed();
    angularSpeed = chassis.getAngularSpeed();
  ...
  public void setAngularAcceleration(double acceleration) {
    ...
    this.chassis.setAcceleration(linearAcceleration, angularAcceleration);
  }
  ...
  public void setLinearSpeed(double speed) {
    linearSpeed = speed;
    this.chassis.setSpeed(linearSpeed, angularSpeed);
   }
  ...
  public void travel(double distance, boolean immediateReturn) {
	...
	double starGyroAngle = gyro.getAngle();  
	...
	System.out.println("X" + (int)(starGyroAngle-gyroAngle) );

	while (chassis.isMoving()){

	 if ((int)(starGyroAngle-gyroAngle) <0 ){

	    	leftM.setSpeed((int) (linearSpeed*1.1));
	    	rightM.setSpeed((int) linearSpeed);
	 }
	 else if ((int)(starGyroAngle-gyroAngle)>0){
	    	leftM.setSpeed((int)linearSpeed);
	    	rightM.setSpeed((int) (linearSpeed*1.1));
	 }
	 else{
	...
	//rotate(starGyroAngle-gyroAngle,false);
  }
  ...
  public void rotate(double angle, boolean immediateReturn) {
	  double starGyroAngle = gyro.getAngle();  
  	System.out.println("Hoek" + (double)(starGyroAngle));

		arc(0,angle, false);
		
	    double gyroAngle = gyro.getAngle()-starGyroAngle;
		Delay.msDelay(500);
	    ...
	    while ((int)(angle-gyroAngle) <0 | (int)(angle-gyroAngle)>0){
	    	...
	    	System.out.println("X" + (double)(angle-gyroAngle) );

	    	arc(0,angle-gyroAngle, false);
	    	Delay.msDelay(500);
	    	gyroAngle = gyro.getAngle()-starGyroAngle;
	    	System.out.println("X" + (double)(angle-gyroAngle) );
  ...
  public void rotateLeft() {
	  rotateImmediateReturn(Double.POSITIVE_INFINITY, true);
  }

  public void rotateRight() {
	  rotateImmediateReturn(Double.NEGATIVE_INFINITY, true);
  }
